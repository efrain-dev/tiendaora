...
SELECT FC.FECHA_VENTA AS FECHA, CE.NOMBRE_CLIENTE AS CLIENTE, USERS.NAME AS EMPLEADO, FC.NO_FACTURA AS FACTURA,SUM(DETV.TOTAL) AS TOTAL
FROM FACTURA_VENTA FC, DETALLE_VENTA DETV, CLIENTE CE, USERS
WHERE (FC.ID_VENTA = DETV.FACTURA_VENTA_ID_VENTA) AND (FC.USERS_ID = USERS.ID) AND (FC.CLIENTE_ID_CLIENTE = CE.ID_CLIENTE) AND  (FC.FECHA_VENTA >= (SYSDATE-30))
GROUP BY FC.FECHA_VENTA, CE.NOMBRE_CLIENTE, USERS.NAME, FC.NO_FACTURA 
...
SELECT FC.FECHA_COMPRA AS FECHA, CE.NOMBRE_PROVEEDOR AS PROVEEDOR, USERS.NAME AS EMPLEADO, FC.NO_COMPRA AS FACTURA,SUM(DETV.TOTAL) AS TOTAL
...
GROUP BY FC.FECHA_COMPRA, CE.NOMBRE_PROVEEDOR, USERS.NAME, FC.NO_COMPRA
HAVING(SUM(DETV.TOTAL)>0)
